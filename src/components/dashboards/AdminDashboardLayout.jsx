...
export const AdminSidebar = ({ children }) => {
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <button
        className="lg:hidden p-3 text-gray-700 hover:text-blue-600 focus:outline-none z-50 fixed top-4 left-4 bg-white/90 backdrop-blur-sm rounded-lg shadow-lg transition-all duration-300 hover:shadow-xl"
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 sm:w-72 bg-gradient-to-b from-slate-900 via-blue-900 to-slate-900 text-white shadow-2xl transform transition-all duration-300 ease-in-out backdrop-blur-xl border-r border-blue-800/30
          ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:min-h-screen`}
      >
        {/* Logo Header */}
        <div className="flex flex-col items-center justify-center h-24 lg:h-28 border-b border-blue-800/30 bg-gradient-to-r from-blue-800/20 to-purple-800/20 relative px-4">
          <Link href="/dashboard/admin" className="group transition-all duration-500 hover:scale-[1.01] mb-2">
            {/* Logo Container with Clean Background - Matching Navbar/Footer */}
          ...
          {/* Text below the logo */}
          <span className="text-white text-xs lg:text-sm font-semibold tracking-wide text-center">Digital Library</span>
          <button
            className="lg:hidden absolute top-3 right-3 text-white hover:text-blue-300 focus:outline-none transition-colors duration-200"
            onClick={toggleSidebar}
          >
        ...
        <nav className="mt-4 lg:mt-6 px-2 lg:px-3 pb-4 max-h-[calc(100vh-6rem)] lg:max-h-[calc(100vh-7rem)] overflow-y-auto">
              ...
              {item.suboptions ? (
                <div>
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 lg:px-4 py-2 lg:py-3 text-gray-200 hover:bg-gradient-to-r hover:from-blue-800/50 hover:to-purple-800/50 transition-all duration-300 rounded-lg lg:rounded-xl group text-sm lg:text-base
                  ...
                  >
                    ...
                    <span className="text-blue-300 group-hover:text-white transition-all duration-300">
                      {expanded.level1 === level1Key ? (
                        <FiChevronDown className="w-4 h-4 lg:w-5 lg:h-5 transition-transform duration-300" />
                      ) : (
                        <FiChevronRight className="w-4 h-4 lg:w-5 lg:h-5 transition-transform duration-300" />
                      )}
                    </span>
                  </button>
                  {expanded.level1 === level1Key && (
                    <ul className="ml-4 lg:ml-6 mt-1 lg:mt-2 space-y-1">
                          ...
                          {suboption.suboptions ? (
                            <div>
                              <button
                                ...
                                className={`w-full flex items-center justify-between px-3 lg:px-4 py-1.5 lg:py-2 text-gray-300 hover:bg-blue-800/30 transition-all duration-200 text-xs lg:text-sm rounded-md lg:rounded-lg group
                              ...
                              >
                                ...
                                <span className="text-blue-300 group-hover:text-white transition-all duration-200">
                                  {expanded.level2 === level2Key ? (
                                    <FiChevronDown className="w-3 h-3 lg:w-4 lg:h-4 transition-transform duration-300" />
                                  ) : (
                                    <FiChevronRight className="w-3 h-3 lg:w-4 lg:h-4 transition-transform duration-300" />
                                  )}
                                </span>
                              </button>
                              {expanded.level2 === level2Key && (
                                <ul className="ml-4 lg:ml-6 mt-1 space-y-1">
                                  {suboption.suboptions.map((child) => (
                                    <li key={child.path}>
                                      <Link
                                        href={child.path}
                                        className={`flex items-center px-3 lg:px-4 py-1.5 lg:py-2 text-gray-300 hover:bg-blue-800/30 transition-all duration-200 text-xs lg:text-sm rounded-md lg:rounded-lg group
                                      ...
                                      >
                                        <span className="w-5 h-5 lg:w-6 lg:h-6 flex items-center justify-center">
                                          <span className="w-1.5 h-1.5 lg:w-2 lg:h-2 bg-blue-400 rounded-full group-hover:bg-blue-300 transition-colors duration-200"></span>
                                        </span>
                                        <span className="ml-1 lg:ml-2">{child.label}</span>
                                      </Link>
                                    </li>
                                  ))}
                                </ul>
                            ...
                            >
                              ...
                              <span className="ml-1 lg:ml-2">{suboption.label}</span>
                            </Link>
                          )}
                        </li>
                      );})}
                ...
                <Link
                  href={item.path}
                  className={`flex items-center px-3 lg:px-4 py-2 lg:py-3 text-gray-200 hover:bg-gradient-to-r hover:from-blue-800/50 hover:to-purple-800/50 transition-all duration-300 rounded-lg lg:rounded-xl group text-sm lg:text-base
                    ${pathname === item.path ? 'bg-gradient-to-r from-blue-700 to-purple-700 text-white shadow-lg' : ''}`}
                  onClick={item.onClick || (() => setIsOpen(false))}
                >
                  <span className="mr-2 lg:mr-3 text-blue-300 group-hover:text-white transition-colors duration-300">{item.icon}</span>
                  <span className="font-medium">{item.label}</span>
                </Link>
              )}
            </li>
    ...
    {/* Main Content */}
    <div className="flex-1 p-3 lg:p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen ml-0 lg:ml-0">
      <div className="bg-white/80 backdrop-blur-sm rounded-xl lg:rounded-2xl shadow-xl p-4 lg:p-8 min-h-[calc(100vh-1.5rem)] lg:min-h-[calc(100vh-3rem)] border border-gray-200/50 mt-16 lg:mt-0">
        {children}
      </div>
    </div>
